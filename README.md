...
> **Note on this delivery:** `artifacts/demo/` was generated by a real, live run of
> `make demo` (provider `google_genai`, model `gemini-flash-latest` — `LLM_PROVIDER` also
> accepts `anthropic` or `openai`) — not fabricated. That run's own log doubles as a live
> demonstration of graceful degradation: Hacker News' keyless API returned `200` for every
> query, while Reddit returned `403` for every query from this sandbox's IP (logged as 28
> `blocked` skips in the brief's methodology section, never a crash) — the same path
> `tests/test_sources_failures.py` exercises offline. All 8 proposed claims passed
> verbatim-quote grounding with zero rejections. One artifact of using a fictional target
> name ("Flowdeck") against a real keyless search API: Hacker News also indexes a real,
> unrelated dev tool called "Flowdock"/"flowdeck.studio", so a couple of the target's
> citations in the committed demo are genuine, verbatim, correctly-attributed quotes about
> that unrelated product rather than about the fictional Flowdeck — a naming collision in
> keyword search, not a grounding failure (every quote is still real and correctly cited to
> its actual source). Re-running with a less common target name avoids this.
